...
func (l *MoveFileLogic) MoveFile(req *types.MoveFileReq) (resp *types.NilResp, err error) {
	...
	data.Set("target_dir", req.TargetDir)

	apiURL := "https://proapi.115.com/open/ufile/move"
	...
	if !result.State {
		return nil, fmt.Errorf("移动文件失败: %s", result.Message)
	}
